"""
aristotle/prove_amm.py
Submit AMM verification condition file to Aristotle and collect results.

Usage:
    cd /path/to/project
    export ARISTOTLE_API_KEY="your-key-here"
    python aristotle/prove_amm.py

The AMM property file (properties/AMM.lean) is submitted as a single Aristotle
job with context [evm_model/Basic.lean, evm_model/AMM.lean].  Results are
written to results/run_amm_<timestamp>.json and the filled Lean output is
saved to results/AMM_proved.lean.
"""
...
AMM_CONTEXT_FILES = [
    ...
    PROJECT_ROOT / "evm_model" / "AMM.lean",
]

AMM_PROPERTY_FILE = PROJECT_ROOT / "properties" / "AMM.lean"
...
def check_files() -> tuple[list[str], Path]:
    """Verify all source files exist. Returns (context_paths, property_file)."""
    context_paths = [str(p) for p in AMM_CONTEXT_FILES]
    for p in AMM_CONTEXT_FILES:
        if not p.exists():
            print(f"ERROR: context file not found: {p}", file=sys.stderr)
            sys.exit(1)
    if not AMM_PROPERTY_FILE.exists():
        print(f"ERROR: property file not found: {AMM_PROPERTY_FILE}", file=sys.stderr)
        sys.exit(1)
    return context_paths, AMM_PROPERTY_FILE
...
async def prove_amm(
    ...
    """Submit the AMM property file to Aristotle, await completion, return record."""
    module_name = prop_file.stem
    output_path = results_dir / f"{module_name}_proved.lean"

    print(f"\n{'='*60}")
    print(f"  Submitting: {prop_file.name}")
    print(f"  Context:    {[Path(p).name for p in context_paths]}")
...
def save_results(results: list[dict], results_dir: Path) -> Path:
    ...
    out_path = results_dir / f"run_amm_{ts}.json"
...
async def main() -> None:
    ...
    context_paths, prop_file = check_files()
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    print(f"Aristotle AMM Verification Runner")
    print(f"Project root: {PROJECT_ROOT}")
    print(f"Context files: {[Path(p).name for p in context_paths]}")
    print(f"Property file: {prop_file.name}")

    record = await prove_amm(prop_file, context_paths, RESULTS_DIR)
    results = [record]
